...
def insert_to_all_games(all_games_list):
    ...
    renamed_games_col = dict(
        zip(
            orig_games_col[0:],
            ("_".join(col.split("_")[0:]) for col in orig_games_col[0:]),
        )
    )
...
def insert_to_all_teams(all_teams_list):
    ...
    renamed_teams_col = dict(
        zip(
            orig_teams_col[0:],
            ("_".join(col.split("_")[0:]) for col in orig_teams_col[0:]),
        )
    )
...
def insert_to_user_stats(user_game_details):
    ...
    pd_user_game_details.to_sql(
        "user_games", con=db_conn, if_exists="append", index=False
    )
...
def update_table(table_name, list_of_cols, list_of_values):
    ...
    update_set = ",".join(
        [f"{col} = '{val}'" for col, val in zip(list_of_cols[0:], list_of_values[0:])]
    )
