...
from scripts.db_insert_values import (
    insert_to_user_stats,
    insert_to_all_games,
    insert_to_all_teams,
)
...
def check_games(date_yesterday):
    ...
    request_games = requests.get(
        url=f"{games_url}{today_date_url_param}", headers=headers
    )
    ...
    all_games_list = [
        f"{res[0] + 1} | {res[1]['home_team']['full_name']} VS {res[1]['visitor_team']['full_name']}"
        for res in enumerate(request_games_data)
    ]
...
def check_matchup(choice, request_games_data):
    ...
    for i in save_to_all_teams_list:
        all_teams_df_temp[i] = pd.concat(
            [
                home_team_df.apply(test_fxn, col=i),
                visitor_team_df.apply(test_fxn, col=i),
            ]
        )
    ...
    while True:
        try:
            ...
            matchup_details = {
                ...
                "home": {"team_name": home_team_name, "team_id": home_team_id},
                "visitor": {"team_name": visitor_team_name, "team_id": visitor_team_id},
            }

            teams = [
                {
                    matchup_details["home"]["team_name"]: matchup_details["home"][
                        "team_id"
                    ]
                },
                {
                    matchup_details["visitor"]["team_name"]: matchup_details["visitor"][
                        "team_id"
                    ]
                },
            ]
            ...
            # is_choice_home
            is_choice_home = (
                True if matchup_details["home"]["team_id"] == user_team else False
            )
...
def check_winner_game(json_matchup_details):
    ...
    team_win = (
        check_win["home"]["team_name"]
        if check_win["team_win"] == check_win["home"]["team_id"]
        else check_win["visitor"]["team_name"]
    )
    print("====== team win =====")
    print(team_win)
    return team_win
